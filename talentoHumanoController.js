...
export const crearCarpeta = async (req, res) => {
    try {
        ...
        // En tu modelo TalentoHumano.js necesitas agregar esta función
        const carpeta = await TalentoHumano.crearCarpeta(nombre);
...
export const obtenerCarpetas = async (req, res) => {
    try {
        // En tu modelo TalentoHumano.js necesitas agregar esta función
        const carpetas = await TalentoHumano.obtenerCarpetas();
...
// SUBIR ARCHIVOS (ACTUALIZADA CON CARPETAS)
export const subirArchivos = async (req, res) => {
    try {
        ...
        const carpetaId = req.body.carpetaId || null; // Nuevo: soporte para carpetas

        for (const file of req.files) {
            const nuevoArchivo = await TalentoHumano.subirArchivo({
                nombre_archivo: file.originalname,
                nombre_guardado: file.filename,
                tipo_archivo: file.mimetype,
                tamaño: file.size,
                usuario_subio: 'Tomas',
                carpeta_id: carpetaId // Nuevo: asociar a carpeta
            });
            ...
            archivosSubidos.push({
                ...
                carpeta_id: nuevoArchivo.carpeta_id // Nuevo
            });
        }
...
// OBTENER ARCHIVOS (ACTUALIZADA CON FILTRO POR CARPETA)
export const obtenerArchivos = async (req, res) => {
    try {
        const { carpetaId } = req.query; // Nuevo: filtro por carpeta
        
        // En tu modelo TalentoHumano.js necesitas modificar obtenerArchivos para aceptar carpetaId
        const archivos = await TalentoHumano.obtenerArchivos(carpetaId);
        ...
        const archivosFormateados = archivos.map(archivo => ({
            ...
            carpeta_id: archivo.carpeta_id, // Nuevo
            url: `/api/talento-humano/archivos/${archivo.id}/ver` // Ruta actualizada
        }));
...
// VER ARCHIVO (SOLO VISUALIZACIÓN) - YA LA TIENES
export const verArchivo = async (req, res) => {
    try {
        const archivo = await TalentoHumano.obtenerArchivoPorId(req.params.id);
...
export const eliminarArchivo = async (req, res) => {
    try {
        ...
        await TalentoHumano.eliminarArchivo(req.params.id);
...
// OBTENER ESTADÍSTICAS (NUEVA FUNCIÓN)
export const obtenerEstadisticas = async (req, res) => {
    try {
        // En tu modelo TalentoHumano.js necesitas agregar esta función
        const stats = await TalentoHumano.obtenerEstadisticas();
        const carpetas = await TalentoHumano.obtenerCarpetas();
