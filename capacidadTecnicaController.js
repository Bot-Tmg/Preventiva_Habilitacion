...
export const crearCarpeta = async (req, res) => {
    try {
        ...
        const carpeta = await CapacidadTecnica.crearCarpeta(nombre);
...
export const obtenerCarpetas = async (req, res) => {
    try {
        const carpetas = await CapacidadTecnica.obtenerCarpetas();
...
export const subirArchivos = async (req, res) => {
    try {
        ...
        for (const file of req.files) {
            const nuevoArchivo = await CapacidadTecnica.subirArchivo({
...
export const obtenerArchivos = async (req, res) => {
    try {
        ...
        const archivos = await CapacidadTecnica.obtenerArchivos(carpetaId);
        ...
        const archivosFormateados = archivos.map(archivo => ({
            ...
            url: `/api/capacidad-tecnica/archivos/${archivo.id}/ver`
        }));
...
export const verArchivo = async (req, res) => {
    try {
        const archivo = await CapacidadTecnica.obtenerArchivoPorId(req.params.id);
        ...
        // ✅ RUTA CORREGIDA: Sin uploadsConfig, ruta directa
        const modulePath = path.join('uploads', 'capacidad_tecnica');
...
export const eliminarArchivo = async (req, res) => {
    try {
        ...
        await CapacidadTecnica.eliminarArchivo(req.params.id);
...
export const obtenerEstadisticas = async (req, res) => {
    try {
        const stats = await CapacidadTecnica.obtenerEstadisticas();
        const carpetas = await CapacidadTecnica.obtenerCarpetas();
