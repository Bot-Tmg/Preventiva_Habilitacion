...
export const crearCarpeta = async (req, res) => {
    try {
        ...
        const carpeta = await HistoriaClinica.crearCarpeta(nombre);
...
export const obtenerCarpetas = async (req, res) => {
    try {
        const carpetas = await HistoriaClinica.obtenerCarpetas();
...
export const subirArchivos = async (req, res) => {
    try {
        ...
        for (const file of req.files) {
            const nuevoArchivo = await HistoriaClinica.subirArchivo({
...
export const obtenerArchivos = async (req, res) => {
    try {
        ...
        const archivos = await HistoriaClinica.obtenerArchivos(carpetaId);
        ...
        const archivosFormateados = archivos.map(archivo => ({
            ...
            url: `/api/historia-clinica/archivos/${archivo.id}/ver`
        }));
...
export const verArchivo = async (req, res) => {
    try {
        const archivo = await HistoriaClinica.obtenerArchivoPorId(req.params.id);
...
export const eliminarArchivo = async (req, res) => {
    try {
        ...
        await HistoriaClinica.eliminarArchivo(req.params.id);
...
export const obtenerEstadisticas = async (req, res) => {
    try {
        const stats = await HistoriaClinica.obtenerEstadisticas();
        const carpetas = await HistoriaClinica.obtenerCarpetas();
